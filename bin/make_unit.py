...
def make_everything(unit):
    global_query = (('SELECT id, name, url, boundary_coords, flyto_lat, flyto_long, flyto_heading, '
                     'flyto_range, flyto_tilt, icon '
                     f'FROM units WHERE id = \'{unit}\''))
    cursor.execute(global_query)
    records = cursor.fetchall()
    unit,name,url,boundary_coords,lat,long,head,flyto_range,tilt,icon = records[0]
    ...
    make_placemark(unit,name,url,lat,long,head,flyto_range,tilt,icon)
...
def make_placemark(unit,name,url,lat,long,head,flyto_range,tilt,icon):
    placemark_outfile = (f'nps-{unit}-placemark.kml')
    placemark_outfile = os.path.join(placemarks,placemark_outfile)
    print(f'Creating placemark kml for {unit} at {placemark_outfile}')
    xml = open(placemark_outfile, 'w')
    xml.write('<?xml version="1.0" encoding="UTF-8"?>\n')
    xml.write(f'<!-- Generated {now} - by make-unit.py -->\n')
    xml.write(('<kml xmlns="http://www.opengis.net/kml/2.2" '
            'xmlns:gx="http://www.google.com/kml/ext/2.2" '
    ...
    xml.write('<Document>\n')
    xml.write(f'  <name>{name}</name>\n')
    xml.write(f'  <StyleMap id="msn_nps-{unit}-placemark-{icon}">\n')
    xml.write('    <Pair>\n')
    xml.write('      <key>normal</key>\n')
    xml.write(f'      <styleUrl>#sn_nps-{unit}-placemark-{icon}</styleUrl>\n')
    xml.write('    </Pair>\n')
    xml.write('    <Pair>\n')
    xml.write('      <key>highlight</key>\n')
    xml.write(f'      <styleUrl>#sh_nps-{unit}-placemark-{icon}</styleUrl>\n')
    xml.write('    </Pair>\n')
    xml.write('  </StyleMap>\n')
    xml.write(f'  <Style id="sh_nps-{unit}-placemark-{icon}">\n')
    xml.write('    <IconStyle>\n')
    xml.write('      <scale>1.5</scale>\n')
    xml.write('      <Icon>\n')
    xml.write('        <href>https://ausborne.net/images/placemark-star.png</href>\n')
    xml.write('      </Icon>\n')
    ...
    xml.write('    </ListStyle>\n')
    xml.write('  </Style>\n')
    xml.write(f'  <Style id="sn_nps-{unit}-placemark-{icon}">\n')
    xml.write('    <IconStyle>\n')
    xml.write('      <scale>2.5</scale>\n')
    xml.write('      <Icon>\n')
    xml.write('        <href>https://ausborne.net/images/placemark-star.png</href>\n')
    xml.write('      </Icon>\n')
    xml.write(('      <hotSpot x="0.5" y="0.5" xunits="fraction" '
            'yunits="fraction"/>\n'))
    xml.write('    </IconStyle>\n')
    xml.write('  <LabelStyle>\n')
    xml.write('    <color>00ffffff</color>\n')
    ...
    xml.write('    </LookAt>\n')
    xml.write(f'    <styleUrl>#msn_nps-{unit}-placemark-{icon}</styleUrl>\n')
    xml.write('    <Point>\n')
